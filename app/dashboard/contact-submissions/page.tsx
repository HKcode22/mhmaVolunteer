...
import { ArrowLeft, Search, Mail, Phone, Clock, MessageSquare, CheckCircle, Eye, ChevronDown, ChevronRight } from "lucide-react";
                        ...
                        <tr className="border-b border-gray-100">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-1">
                              {!i.read && (
                                <button onClick={(e) => { e.stopPropagation(); markContactSubmissionRead(i.id!).then(() => setItems(prev => prev.map(x => x.id === i.id ? { ...x, read: true } : x))); }}
                                  className="p-1.5 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors" title="Mark as read">
                                  <CheckCircle className="w-4 h-4" />
                                </button>
                              )}
                              <button onClick={(e) => { e.stopPropagation(); if (!confirm("Delete this submission?")) return; deleteContactSubmission(i.id!).then(() => setItems(prev => prev.filter(x => x.id !== i.id))); }}
                            ...
                            <td colSpan={7} className="px-6 py-4 bg-gray-50 border-b border-gray-200">
                              <div className="max-w-3xl">
                                ...
                                <div className="flex items-center gap-4 mt-3 text-xs text-gray-400">
                                  ...
                                  <span>Submitted: {i.createdAt?.toDate?.()?.toLocaleString() || ""}</span>
                                </div>
                              </div>
                            </td>
                          </tr>
